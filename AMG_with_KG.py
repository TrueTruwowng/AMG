...
class AMG_RAG_System:
    ...
    def __init__(self, use_openai: bool = True, openai_key: str = None):
        ...
        api_key=openai_key,
        base_url="http://112.137.129.245:33038/v1",
        temperature=0,
        )
            print('connected')
            ...
            if ChatOllama is not None:
                self.llm = ChatOllama(
                model="qwen2.5:14b", 
                openai_api_base="http://112.137.129.245:33034/v1", 
                temperature=0.0
                )
                print('connected')
            else:
    ...
    def _setup_chains(self):
        ...
        relation_schemas = [
            ResponseSchema(
                name="relationships",
                description="List of relationship objects, each containing entityA, entityB, relationship_type, confidence_score, and evidence",
                type="array"
            )
        ]
        ...
        self.relation_extractor = PromptTemplate(
            ...
            Provide relationships in this exact JSON format, return JSON object with this exact structure::
            {{
                "relationships": [
                    {{
                        "entityA": "{entity1}",
